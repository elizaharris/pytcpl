...
def mc4_mthds(mthd, df):
    ...
    """
    cndx = df['cndx'].isin([1, 2]) # Mask to data from the lowest concentrations
    wllt_t = df['wllt'] == 't' # Mask to well type = t for the test compound (others are things like blanks, etc.)
    mask = df.loc[cndx & wllt_t, 'resp'] # Get the response for these masks
