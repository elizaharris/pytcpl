...
def process(assay_endpoint_info, df, config, logger):
    ...
    cutoffs = get_cutoff() # Fetch these from the parquet file saved in fetch_raw_data() 
    cutoff = cutoffs.iloc[0]['cutoff']
    onesd = cutoffs.iloc[0]['onesd']
    bmad = cutoffs.iloc[0]['bmad']

    def remove_outliers(conc,resp):
        # Cannot use Rosner's test; not recommended with less than 25 data points
        outliers = conc*0
        conc_abs = 10**conc
        return(outliers==1)
    ...
    def fit(series):
        ...
        for fit_model in config['curve_fit_models']:
            mod = get_model(fit_model)
            x0 = mod('x0')(signal_direction_is_bidirectional, conc, resp)
            bounds = mod('bounds_bidirectional')(conc, resp) if signal_direction_is_bidirectional else get_model(fit_model)('bounds')(conc, resp)
            args = (conc, resp, mod('fun'))
            ...
            pars = {k: v for k, v in zip(mod('params'), fit_result.x)}
            ll = -fit_result.fun
            aic = 2 * len(pars) - 2 * ll
    ...
    def hit(series):
        ...
        if params is None:
            ...
            rel_likelihood = np.exp((aics[best_aic_model] - aics["cnst"]) / 2) if aics[best_aic_model] <= aics[
                "cnst"] else 1 # likelihood compared to constant model
            ...
            if 0: # to troubleshoot, run manually
                fig, ax = plt.subplots()
                ax.plot(conc,resp,"bx")
                ax.plot(conc,pred,"r")
                ax.set_title(best_aic_model)
                ax.set_xlabel('conc')
                ax.set_ylabel('resp')
                plt.savefig(os.path.join(ROOT_DIR, '../../figs/tmp.png'))  
            ...
            # The inverse of the function is used to relate back to all the required outputs
        ...
        # These are the (somewhat arbitrary) fitcall categories...
    ...
    nj = config['n_jobs']
    p = nj != 1
    if config["enable_outlier_removal"]==1:
        for id in np.unique(df["spid"]):
            r = np.where(df["spid"]==id)[0]
            ### CONTINUE from here:
            # Indexing is wrong, fix and check it works, and fit the outlier function! Use a spline as a basis?
            outliers = remove_outliers(conc = df["logc"].iloc[r],resp = df["resp"].iloc[r])
            tmp = [ df["resp"].iloc[x] = np.nan for x in r[outliers] ]
    sample_groups = df.groupby(['aeid', 'spid']) # Group by assay and sample (eg. chemical) id
    ...
    # Check the dataseries can be processed, eg. concentrations high enough and so on
    # Each series is marked as fit or not 
    ...
    # Curve-Fitting (on the datasets that can be fit)
    ...
    fit_params = Parallel(n_jobs=nj)(delayed(fit)(i) for _, i in it) if p else [fit(i) for _, i in it] # fit each of the chosen curves and return params, aic, etc.
