...
def launch(config, config_path):
    ...
    """
    # This line needs running when using the script for troubleshooting
    init_config(config)
    ...
    # This line needs running when using the script for troubleshooting
    # instance_id = 0; instances_total = 1; 
...
def launch_script(config, config_path):
    """
    Short version of the above for troubleshooting in script mode
    """
    # This line needs running when using the script for troubleshooting
    init_config(config)
...
def fetch_raw_data():
    ...
    for mthd in load_method(lvl=4, aeid=AEID, config=CONFIG) + other_mthds: # Retrieve various values according to mc4 processing, such as bmad, sd of low concs, etc.
    ...
    bmed = values['bmed.aeid.lowconc.twells'] # Median response in low conc wells
    sd = values['onesd.aeid.lowconc.twells'] # Stdev of response in low conc wells

    cutoffs = [mc5_mthds(mthd, bmad) for mthd in load_method(lvl=5, aeid=AEID, config=CONFIG)] # Get the cutoff for the models in mc5 processing
...
def write_output(df):
    ...
    """
    df = get_metadata(df) # Adds some more metadata columns and fixes some issues with chemical names
    df = subset_chemicals(df) # Take the highest hit call if a chemical is duplicated
...
def get_metadata(df):
    ...
    """
    if sum(df.columns == 'dsstox_substance_id')>0:
