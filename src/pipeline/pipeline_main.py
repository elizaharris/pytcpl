...
# parent_dir = os.path.abspath(os.path.join("/data/eliza/pytcpl/src/pipeline", "../.."))
parent_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), "../.."))
sys.path.append(parent_dir)

from src.pipeline.pipeline_constants import ROOT_DIR, PROFILER_PATH, LOG_DIR_PATH
from src.pipeline.pipeline_helper import load_config, prolog, launch, launch_script, fetch_raw_data, bye, write_output, epilog
from src.pipeline.pipeline_core import process

def pipeline(config, config_path):
    """
    ...
    """
    instance_id, instances_total, aeid_list, logger = launch_script(config, config_path)
